...
zmq::tcp_address_t::tcp_address_t (const sockaddr *sa, socklen_t sa_len)
{
    ...
    memset (&address, 0, sizeof (address));
    if (sa->sa_family == AF_INET && (unsigned int)sa_len >= (socklen_t) sizeof (address.ipv4))
        memcpy(&address.ipv4, sa, sizeof (address.ipv4));
    else 
    if (sa->sa_family == AF_INET6 && (unsigned int)sa_len >= (socklen_t) sizeof (address.ipv6))
...
{
    zmq_assert (address_mask != -1 && ss != NULL && (unsigned int)ss_len >= (socklen_t) sizeof (struct sockaddr));
